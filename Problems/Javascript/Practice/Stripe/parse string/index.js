...
function parse_accept_language(acceptLanguageHeaderStr, supportedLanguages = []) {
  const headerList = acceptLanguageHeaderStr.split(", ");
  const result = headerList.reduce((acc, curr) => {
    if (supportedLanguages.includes(curr)) {
      return [...acc, curr];
    }
    return acc;
  }, []);
...
const result1 = parse_accept_language(
...
// returns: ["en-US", "fr-FR"]
const result2 = parse_accept_language("fr-CA, fr-FR", ["en-US", "fr-FR"]);
// returns: ["fr-FR"]
const result3 = parse_accept_language("en-US", ["en-US", "fr-CA"]);
// returns: ["en-US"]
